...
will run the test specified by BINARY and output results in the output/ folder. You can look at the suite of different tests
...
than software simulation. A key point of FireSim is that it is cycle-accurate since it actually modes things like DRAM access
...
The Hammer flow we have used throughout the semester is integrated into Chipyard. A project setup similar to the ones we have previously 
used in `chipyard/vlsi`. In this directory, run
...
correspond to the total memories in `top.mems.conf` (the names of the memory blocks will match the Verilog modules names).

***Q: What is the breakdown of ASAP7 SRAM blocks for each of the memories in the desin? (this can be found by looking at the files described above.)***
...
way before. Tha Hammer entry point runs through `example-vlsi` (setup to easily include additional custom Hammer hooks) and our Hammer config is in
...
sure that you don't lose your run if you lose your connection of log off. When it completes, you can look at the results just like before in `syn-rundir/reports/`
...
As noted in a previous lab, if you edit an input yml/json file, the Hammer Make include file will detect this and re-run the flow. If you want to edit a 
...
You can open up the final design in Innovus using `par-rundir/generated-scripts/open_chip` just like in the previous labs. Note that we are using "express"
